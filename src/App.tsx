...
export default function App() {
  ...
  const renderAvatar = (player: Player, isActive: boolean) => {
    return (
      <div className={`avatar-container ${isActive ? 'ring-2 ring-[#32ff7e]' : ''}`}>
        {player.avatar ? (
  ...
  // Sorted Local Player Hand
  const sortedHand = [...localPlayer.hand];
  if (sortBy === 'color') {
    const colorOrder: Record<CardColor, number> = { red: 1, yellow: 2, green: 3, blue: 4, wild: 5 };
    sortedHand.sort((a, b) => {
      const cDiff = colorOrder[a.color] - colorOrder[b.color];
      if (cDiff !== 0) return cDiff;
      return a.value.localeCompare(b.value);
    });
  } else if (sortBy === 'value') {
    sortedHand.sort((a, b) => a.value.localeCompare(b.value));
  }
      ...
      <div className="w-full max-w-5xl flex items-center justify-between px-2 mb-2">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold transition-colors border border-slate-700"
          >
            <History className="w-3.5 h-3.5 text-[#18dcff]" />
            <span className="hidden sm:inline">Lịch sử</span> ({currentRoom.logs.length})
          </button>
              ...
              <div className="flex flex-col items-center gap-1.5">
                <div
                  className={`player-tag ${
                    topOpponent.id === activePlayer?.id ? 'active' : ''
                  }`}
                >
          ...
          <div id="slot-left" className="player-slot left">
            {leftOpponent ? (
              <div className="flex flex-col items-center gap-1.5">
                <div
                  className={`player-tag ${
                    leftOpponent.id === activePlayer?.id ? 'active' : ''
                  }`}
                >
          ...
          <div id="slot-right" className="player-slot right">
            {rightOpponent ? (
              <div className="flex flex-col items-center gap-1.5">
                <div
                  className={`player-tag ${
                    rightOpponent.id === activePlayer?.id ? 'active' : ''
                  }`}
                >
